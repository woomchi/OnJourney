...
> 마지막 업데이트: 2026-09-18  
> **프로젝트 상태: 🔒 개발 종료 (Archived)** — 회고록: [`docs/history/retrospective.md`](./retrospective.md)
...
| **인천 버스 실시간 도착** | ✅ | `IncheonBusService` 연동 |
| **대전 버스·도시철도 실시간 도착** | ✅ | 대전 버스·지하철 실시간 정보 통합 |
| **버스 노선도 뷰** | ✅ | 버스 노선 실시간 현황 시트/모달 |
| **지하철 분기선 관리** | ✅ | 분기 노선 분기 방향 관리 방식 도입 |
| **여정 공유** | ✅ | `is_public` RLS 확장 + `/share/[journeyId]` SSR 뷰어 + 공유 토글/복사 모달 |
| **URL 기반 UI 상태 관리** | ✅ | `useUrlState` — 뒤로가기/앞으로가기 + 딥링크 지원 |
...
| 역간거리 DB 자동 동기화 | 공공 API 연동 기반 주간 자동 업데이트 |
| SEO 최적화 | OpenGraph, JSON-LD, sitemap.xml 추가 |
...
### 2026-09-18 — Phase 11: 버스 노선 위상 매칭 정교화
- **307번 버스 상행·하행 승차 오판정 문제 분석·해결** — `resolveActualBoardingInfo` 다차원 스코어링 로직 보강
- **`findBestMatchingStationIndex` 함수 신설** — ID·ARS 일치 → 명칭 일치 → 퍼지 → GPS 좌표 근접 폴백 4단계 우선순위
- **`tests/transit/busStationTopologyMatching.test.ts` 작성** — 4개 테스트 케이스, 타입 오류 0건, 테스트 전원 통과

### 2026-08-26 ~ 08-28 — Phase 10: 리팩토링 & 공유/URL 라우팅 (최종)
- **대규모 리팩토링** (`01cd6d4`) — 프로젝트 전반 코드 구조 정리 및 컴포넌트 세부 분리
- **URL 라우팅 상태 관리 도입** (`eaa22cc`) — `useUrlState` 기반 딥링크, 뒤로가기/앞으로가기 연동
- **지하철 실시간 분기선 관리 방식 도입** (`334cdf1`) — 분기 노선 방향 관리 로직
- **지하철 실시간 표기명 파싱 보완** (`f4b4e5e`) — 추가 엣지케이스 처리
- 여정 공유 기능 (`is_public` RLS + `/share/[journeyId]` SSR 뷰어) 완성
- 에이전트 명세(`AGENTS.md`) 최신화 및 테스트 파일 보강

### 2026-08-18 ~ 08-19 — Phase 9 연장: 실시간 커버리지 확대 & 버스 노선도 뷰
- **인천 버스 실시간 API 연동** (`6da140a`) — `IncheonBusService`
- **대전 버스·지하철 실시간 정보 도입** (`f6b164a`) — 대전 시내버스 + 도시철도 통합
- **버스 노선도 뷰 도입** (`f6b164a`) — 버스 노선 실시간 현황 시트 구현
- **PWA 실시간 정보 UI/UX 개선** (`7833c75`) — 바텀 시트 실시간 정보 뷰 개편
- 지하철 역간거리 정보 업데이트 및 파일 정리 (`9f09bc6`)
- 지하철 실시간 도착 정보 시스템 리팩토링 (`64d076a`)
