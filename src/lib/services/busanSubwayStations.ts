/**
 * @fileoverview 부산 도시철도 역사 메타데이터 및 순수 유틸리티
 *
 * - Node.js 내장 모듈(fs, path 등)에 의존하지 않아 클라이언트 컴포넌트 및 브라우저 환경에서 안전하게 임포트 가능합니다.
 */
...
export const TRANSFER_STATIONS: Record<string, string[]> = {
  '서면': ['119', '219'],
  '연산': ['123', '305'],
  '동래': ['125', '402'],
  '수영': ['208', '301'],
...
/**
 * 역명과 호선으로 부산 지하철 역코드(scode)를 획득합니다 (클라이언트 안전).
 */
...
/**
 * 출발역과 목적역(또는 하차역)의 역코드를 기반으로 운행 방향(UP/DOWN)을 판별합니다.
 */
export function inferBusanDirection(
  lineNum: string,
  startStation?: string,
  destStation?: string
): 'UP' | 'DOWN' | null {
  ...
  const startCode = getBusanStationCode(startStation, lineNum);
  const endCode = getBusanStationCode(destStation, lineNum);
  if (!startCode || !endCode || startCode === endCode) return null;

  const s = parseInt(startCode, 10);
  const e = parseInt(endCode, 10);
  if (isNaN(s) || isNaN(e)) return null;

  // 1호선: 번호 증가(다대포 95 -> 노포 134)가 UP, 감소가 DOWN
  if (lineNum === '1') {
    return e > s ? 'UP' : 'DOWN';
  }
  // 2, 3, 4호선: 번호 감소가 UP, 증가가 DOWN
  return e < s ? 'UP' : 'DOWN';
}
...
/**
 * 공공데이터포털 지하철 API 통합 인증키 획득 (하위 호환 유지)
 */
