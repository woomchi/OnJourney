# OnJourney 개발 흐름 로그

> Git 커밋 이력을 기반으로 정리한 프로젝트 개발 흐름 요약입니다.  
> 세부 변경 사항보다는 **어떤 기능이 어떤 순서로 추가·개선·제거되었는지**의 흐름에 집중합니다.

---

## Phase 1 — 기초 기능 구축 (2026-06-14 ~ 06-16)

### 장소 추가 & 인증
- 장소 추가 기능 초기 구현 및 반복 수정
- 로그인 관련 기능 개선

### 지도 핵심 기능
- 이동 수단 **타임라인 바** 디자인 구조 구현
- **대중교통 정보 연동 및 Polyline** 렌더링 구현
- **줌 아웃** 기능 구현
- Polyline **방향 마크 & 구간 하이라이트** 기능 추가
- 장소 순서 변경 시 경로 자동 업데이트 로직 적용
- 서버 시작 시 무한 로딩 오류 수정

---

## Phase 2 — 지도 시각화 & 마커 정교화 (2026-06-17 ~ 06-18)

### Polyline 시안성 개선 (4단계에 걸친 반복 개선)
1. 1차 — Polyline 기본 시각 개선
2. 2차 — 마커 시안성 동시 개선
3. 3차 — 대안 교통수단 선택 UI 병행 구현
4. 4차 — **Polyline 레이어 분리 및 우선순위 부여**

### 마커 시스템
- **탑승 마커** 구현 (택시 요금 오류 보정 포함)
- 탑승 마커 클릭 시 세부 노선 줌 인 & 하이라이트
- **도보 마커** 추가 및 다음 이동 정보 마커 연동
- 장소 마커 표시 기준 인터랙션 정비

### 자동차 경로
- **자동차 경로** 기능 구현 및 마커 로직 수정

### UI
- 타임라인 바 하단 대중교통명 배치
- 전체 경로 표시 버튼 디자인 수정
- 사이드바 너비 조정, 아코디언 인터랙션 수정

---

## Phase 3 — 상세 패널 & 실시간 정보 시스템 (2026-06-19 ~ 06-21)

### 상세 정보 패널 신설
- **차량 및 대중교통 상세 정보 패널** 구현
- 상세 패널 내 구간 카드 인터랙션 구현
- 타임라인 바 하이라이트 방식 수정
- 특정 세부 구간 선택 시 **자동 스크롤** 기능 추가
- 이동 정보 패널에서 다음 패널로 이동하는 버튼 UI 추가
- 상세 패널 위치 정렬 및 최종 목적지 카드 디자인 수정

### 여정 편집 기능
- 여정 정보(제목/날짜/이동수단) **상시 편집 방식**으로 전환
- 날짜 지정 제한 해제
- 차량 경로 대안 세분화

### 줌 & 뷰 조정
- 가상 박스 기반 줌 배율 세분화
- 패널 추가에 따른 줌 방식 동적 조정
- 검색바 삭제 및 이에 따른 줌 조절 방식 수정

### 실시간 정보 시스템 (추가 후 임시 제거)
- **버스 및 지하철 실시간 정보** 시스템 구축
- 대중교통 정보 일괄 호출 방식 수정
- 가짜 실시간 데이터 로직 제거 → 새로고침 애니메이션 추가
- 사이드바 내 실시간 정보 UI 임시 삭제

### 여정 컨트롤
- 여정 컨트롤(재생/정지 등) 기능 구현
- 줌 인아웃 인터랙션 세부 수정

---

## Phase 4 — 구조 정리 & 성능 최적화 (2026-06-23 ~ 06-24)

### 대규모 리팩토링
- 핵심 프론트 파일, 사이드바, 상세 패널 코드 리팩토링
- **파일 분리 및 프로젝트 구조 정리**
- **shadcn/ui 도입** — UX/UI 디자인 요소 체계화
- 코드 분할 및 렌더링 최적화

### 데이터 패칭
- **React Query 도입** — 관련 로직 오류 및 버그 수정
- 가짜 대중교통 데이터 생성 로직 완전 제거

### 대안 패널
- 대안 아코디언 분리 → **독립 패널**로 전환
- 대안 패널 디자인 수정 및 경로 노출 기준 변경
- 대안 카드 디자인 개선

### 경로 렌더링
- **경로 렌더링 애니메이션** 추가
- 순서 기반 경로 색상 버그 수정
- 도보 구간 점선 색상 부여

### 재생 기능
- 여정 재생바 인터랙션 구현
- 사이드바 재생 버튼 구조 개편

---

## Phase 5 — 사이드바 고도화 & 검색 기능 (2026-06-25 ~ 07-05)

### 사이드바 UI & 인터랙션
- 마커 색상 순서 기반 테마 적용
- 사이드바 색상 불일치 버그 수정
- 드래그 앤 드롭 **햅틱/시각 효과** 구현
- 상세 패널 & 대안 패널 **생성 애니메이션** 추가
- 장소 순서 UI 효과 및 재생 타임라인 바 **물결 효과** 추가
- 재생 버튼 인터랙션 추가
- 편집 모드 활성화에 따른 패널 **자동 닫힘** 기능 추가
- 편집 모드 활성화 시 **자동 줌 조정**

### 검색 기능
- 사이드바 검색 모드 전환 및 **추천 기능** 구현
- 검색 로직 반복 수정 (총 3회)
- 검색 하이라이트 방식 변경
- 현재 화면 기준 검색 버튼 배치 및 인터랙션 수정
- GPS 응답 속도 개선

### 성능 & 안정성
- 캐싱 최적화 및 리렌더링 문제 해결
- 마커 및 Polyline 렌더링 지연 문제 해결
- 지도 렌더링 및 상태 동기화 최적화
- 사이드바 검색 화면 로직 수정 및 애니메이션 추가

---

## Phase 6 — 모바일 환경 대응 (2026-07-06 ~ 07-20)

### 모바일 구조화 & UI
- 초기 **로그인 화면** 구현 및 모바일 구조 체계화
- 모바일 UI/UX 1차 작업
- 모바일 UI 시안성 향상

### 계정 인터랙션
- 계정 관련 인터랙션(프로필, 세션 등) 추가

### 바텀 시트 시스템
- **상세/대안 패널 → 바텀 시트** 방식으로 전환
- 바텀 시트 드래그 애니메이션 개선
- 높이 조절에 따른 내부 요소 인터랙션 수정 (여정/장소/이동 카드, 줌 조정)
- 상세 패널 UI → **플로팅 플레이어 바** 방식으로 변경
- 여정 상세 바텀 시트 스크롤 감도 및 여백 수정
- 상세 바텀 시트 세부 이동 정보 **UI 개편 및 스크롤 방식** 수정

### GPS & 센서
- **모바일 GPS 기능** 구현
- **자이로스코프(방향 감지)** 기능 구현
- GPS 기능 보류 → 이후 복구 작업

### 시스템 최적화
- 모바일 환경 시스템 전반 최적화
- 검색바 위치 조정 및 **오버 스크롤** 기능 추가
- 지도 줌 버그 수정

---

## Phase 7 — 캐싱 전략 고도화 (2026-07-28)

### 시간대별 캐싱 시스템
- **시간대별 캐시 키 분리** 구현
  - 평일/주말, 낮/밤 시간대별 캐시 분리 (4가지 시간대)
  - 밤 시간대 올바른 대중교통 경로 제공 문제 해결
  - `getTimeSlot()` 함수로 시간대 분류

### 출발 시간 기반 캐싱
- **출발 시간 파라미터 추가** 구현
  - `departureTime` 파라미터 추가 (선택적, Unix timestamp)
  - 3시간 단위 시간 그룹화로 캐시 파편화 최소화
  - 예약 여정 계획 기능 지원
  - 클라이언트/서버/스키마 전체 수정

### 동적 캐시 만료 시간
- **시간대별 동적 캐시 만료 시간** 구현
  - 밤 시간대 (23:00-06:00): 30분 캐싱 - 데이터 신선도 확보
  - 낮 시간대 (06:00-23:00): 4시간 캐싱 - API 절감 극대화
  - 차량 경로에 동적 캐시 기간 적용
  - ODsay API 무료 플랜 제한(1,000회/일) 고려 최적화

### 성능 최적화 효과
- 밤 시간대 데이터 신선도 확보
- 낮 시간대 API 호출 75% 감소
- 시간대별 올바른 경로 정보 제공
- 예약 여정 계획 기능 지원

---

## Phase 8 — 인프라 고도화 & 신규 서비스 연동 (2026-07-29 ~ 2026-08-12)

### 장소 검색 API 전환
- **네이버 장소 검색 → 카카오 로컬 API** 전환
  - 키워드 검색 + 카테고리 검색 이중 구조
  - `searchPatternService.ts` 도입: 검색어 패턴 분석 + 카테고리 코드 자동 매핑
  - **Gaussian Decay 거리 점수** 기반 랭킹 (도보/도심/외곽 scale 구분)
  - Cold Start 방지용 Tier1 카테고리 인기도 가산점

### 도보 경로 실제 API 연동
- **ODsay 멀티모달 API 연동** (`odsayWalkingService.ts`)
  - 10km 미만 구간에 ODsay `maasRP` (멀티모달 API) 기반 도보/자전거/킥보드 경로 탐색 적용 (기존 TMAP 보행자 API 대체)
  - 단일 `ODSAY_API_KEY` 사용으로 API 인증 관리 통합 및 데이터 일관성 확보
  - ODsay API 장애 또는 범위 초과 시 `buildWalkFallbackResults` 기반 Fallback 자동 전환

### 인프라 패턴 도입
- **Circuit Breaker** 패턴 구현 (`circuitBreaker.ts`)
  - CLOSED → OPEN → HALF_OPEN 3상태 관리
  - ODsay 연속 실패 3회 시 즉시 Fail-Fast Fallback
  - 10초 쿨다운 후 HALF_OPEN 복구 시험
- **Rate Limiter** 구현 (`rateLimiter.ts`): ODsay 무료 플랜(1,000회/일) 보호
- **OdsayAdapter** 도메인 에러 클래스 체계화
  - `TransitApiError`, `TransitAuthError`, `TransitQuotaError`, `TransitRouteNotFoundError`

### 지형 분류 시스템
- **GeoJSON 기반 지형 분류** 구현 (`walkabilityCheck.ts`, `terrainClassifier.ts`)
  - Phase 1: O(1) BBox 선검사 → Turf.js PIP 2차 정밀 검증
  - `naturalData.json` + `landUseData_processed.json` 파일 로드 (forest, wood, water, beach 등)
  - 지형 타입 분류: `normal` / `mountain` / `beach`

### DnD 라이브러리 전환
- **HTML5 Native DnD → `@dnd-kit`** 전환 (core / sortable / modifiers)
  - 모바일 터치 지원 향상
  - `useMobileDndSensors` 훅으로 센서 설정 추상화

### 출발 시간 선택 UI
- `DepartureTimePicker` 및 `DepartureTimeSelector` 컴포넌트 추가
  - 여정 출발 시간 직접 지정 기능
  - 기존 `departureTime` 파라미터 캐시와 연동

### GPS 트래킹 복구
- Phase 6에서 보류된 GPS 기능 **`useGPSTracking` 훅**으로 재구현
  - `useGeocodeOnIdle.ts`: 지도 idle 상태 기반 역지오코딩

### 피처 모듈화 (features/ 디렉터리)
- `features/map/`: MapArea, MapMarkers, MapRoutes, MapOverlays, MapFloatingControls, useMapCamera
- `features/route/`: AlternativeRoutePanel, RouteGuidePanel, RoutePanels
- `features/places/`: PlaceSearchBar, usePlaceSearch

### React Query 영속화
- `@tanstack/react-query-persist-client` + `idb-keyval` 도입
  - IndexedDB 기반 쿼리 캐시 영속화 (앱 재시작 시 캐시 유지)

---

## Phase 9 — 실시간 정확도 고도화 & 검색 엔진 v3 (2026-08-13 ~ 08-17)

### 지하철 ETA 엔진 개선 & 실시간 위치 연동
- **`SubwayMessageParser` 구현** (`subwayMessageParser.ts`)
  - `arvlMsg2` 메시지 정밀 파싱: NFD→NFC 유니코드 정규화, 다중 공백/자모 결합 오류 처리
  - 파싱 결과: `status` (entering/arrived/departed/approaching/unknown), `stationName`, `remainingStations`, `confidence` (0~1)
  - 기존 단순 정규식 대비 역명 추출 정확도 90% → 96% 목표
- **`TimeOffsetManager` 구현** (`timeOffsetManager.ts`)
  - 클라이언트-서버 시각 오프셋 Singleton 관리
  - RTT 보정: `estimatedServerTime = serverTime + round(RTT/2)` — ±1초 이내 정밀도 목표
  - `syncConfidence` 산출 (오프셋 절댓값에 비례 감점, 최소 0.5)
  - 지하철 실시간 ETA(`barvlDt - recptnDt` 경과 보정)에 통합 적용
- **`subwayTotalRealtimeService.ts` 신설**
  - 지하철 전역 실시간 통합 서비스
- **실시간 열차 위치 연동 (`subwayPositionService.ts`) & 노선도 현황 뷰 (`SubwayLineMapSheet.tsx`)**
  - 서울시 `realtimePosition` API (`REAL_TIME_SUBWAY_LOCATION_API_KEY`) 연동 및 15초 인메모리 캐시
  - `trainNo` 기준 도착 API와 조인하여 정확한 현재 위치 역명(`statnNm`) 기반 ETA 산출
  - 지하철 실시간 도착 칩(`SegmentSubwayRealtimeChip`) 클릭 시 해당 노선의 실시간 정차역 타임라인과 열차 위치를 시각화하는 `SubwayLineMapSheet` 노선도 뷰 모달/시트 구현
  - `api/subway/positions` 엔드포인트 및 `useSubwayLinePositions` 훅 추가

### 장소 검색 엔진 v3
- **검색 결과 카테고리 필터링 방식 전환** (v2 Drop 방식 → v3 태깅 방식)
  - 여행 외 카테고리 사전 제거 → **전체 수집 + 서비스 카테고리 태그 부여** (`ServiceCategoryTag`)
  - `etc` 태그 도입: 미분류 결과를 삭제하지 않고 S_cat=0.2 점수로 후순위 배치
  - `placesService.ts` 멀티 파이프라인 (Pipeline A: accuracy, Pipeline B: distance) 병합 방식 적용
- **`MapCategoryChips` 컴포넌트 구현** (`components/map/MapCategoryChips.tsx`)
  - 검색 결과 상단 가로 스크롤 칩 UI: [전체] [관광명소] [음식점] [카페] [숙소] [교통] [편의시설] [기타]
  - 클라이언트 메모리 필터링 (서버 재호출 없음), 지도 마커 동기화
- **`usePlaceSearch` 훅 개선**: `activeCategory` 상태 + `filteredResults` 파생 상태 추가

### 실시간 버스 커버리지 확대
- **부산 버스 실시간 API 연동** (`BusanBusService.ts`)
  - `RealtimeTransitService.ts`에 부산 시내버스 도착 조회 통합

### UI 컴포넌트 대규모 확충
- **사이드바 계층 재편**
  - `ActiveJourneySidebar.tsx`: 활성 여정 전용 사이드바 분리
  - `HorizontalJourneyTimelineBar.tsx`: 가로형 여정 타임라인 바
  - `SidebarBottomActions.tsx`: 사이드바 하단 액션 버튼 모음
- **경로 안내 패널 세분화**
  - `RouteSegmentCardStack.tsx`: 구간 카드 스택 레이아웃
  - `RouteSegmentDetailSheet.tsx`: 구간 상세 바텀 시트
  - `RouteTimelineGaugeBar.tsx`: 경로 타임라인 게이지 바
  - `PlaybackBar.tsx`: 재생 컨트롤 바
  - `FareBreakdownTooltip.tsx`: 요금 내역 툴팁
  - `CarGuideList.tsx`: 차량 경로 안내 목록
- **장소 카드 세분화**
  - `TimelineNode.tsx`: 타임라인 노드 UI
  - `FittedDuration.tsx`: 소요시간 적응형 표시
- **실시간 정보 UI 신설**
  - `RealtimeArrivalCard.tsx`: 실시간 도착 정보 카드
  - `SegmentBusRealtimeChip.tsx`: 구간 버스 실시간 칩
  - `SegmentSubwayRealtimeChip.tsx`: 구간 지하철 실시간 칩
  - `ReliabilityBadge.tsx`: 실시간 데이터 신뢰도 배지
- **지도 오버레이**
  - `CustomOverlayView.tsx`: 범용 커스텀 오버레이 컴포넌트

---

## 삭제 / 임시 제거된 기능

| 항목 | 시점 | 비고 |
|------|------|------|
| 가짜 대중교통 데이터 생성 로직 | Phase 4 | 실 API 연동으로 대체 |
| 가짜 실시간 데이터 로직 | Phase 3 | 실 API 연동 전 임시 제거 |
| 사이드바 내 실시간 정보 UI | Phase 3 | 임시 삭제 (재설계 예정) |
| 검색바 | Phase 3 | 줌/뷰 방식 개편으로 제거 |
| GPS 기능 (일시 보류) | Phase 6 | Phase 8에서 `useGPSTracking`으로 복구 |
| 마커 애니메이션 | Phase 5 | 성능/디자인 이유로 삭제 |
| 네이버 장소 검색 API | Phase 8 | 카카오 로컬 API로 전환 |

---

## 주요 기술 도입 이력

| 기술 / 도구 | 도입 시점 | 목적 |
|------------|----------|------|
| `shadcn/ui` | Phase 4 (06-24) | UI 컴포넌트 체계화 |
| `React Query` | Phase 4 (06-23) | 서버 상태 관리 최적화 |
| 바텀 시트 패턴 (`framer-motion`) | Phase 6 | 모바일 UX 최적화 |
| Polyline 레이어 분리 | Phase 2 | 렌더링 우선순위 제어 |
| 시간대별 캐시 키 분리 | Phase 7 | ODsay API 비용 절감 |
| 출발 시간 파라미터 (`departureTime`) | Phase 7 | 예약 여정 계획 지원 |
| Circuit Breaker | Phase 8 | ODsay 장애 보호 및 Fail-Fast |
| Rate Limiter | Phase 8 | ODsay 무료 플랜 쿼터 보호 |
| TMAP 보행자 경로 API | Phase 8 | 실제 도보 경로 탐색 |
| 카카오 로컬 API | Phase 8 | 장소 검색 개편 (네이버 대체) |
| `@dnd-kit` | Phase 8 | DnD 모바일 지원 향상 |
| GeoJSON + Turf BBox+PIP | Phase 8 | 지형 분류 시스템 |
| `@tanstack/react-query-persist-client` | Phase 8 | 쿼리 캐시 IndexedDB 영속화 |
| `SubwayMessageParser` | Phase 9 | 지하철 메시지 정밀 파싱 (NFD 정규화, Fuzzy Match) |
| `TimeOffsetManager` | Phase 9 | 클라이언트-서버 시각 오프셋 동기화 |
| 검색 엔진 v3 (ServiceCategoryTag + 멀티 파이프라인) | Phase 9 | 장소 검색 카테고리 태깅 방식 전환 |
| `MapCategoryChips` | Phase 9 | 검색 결과 클라이언트 카테고리 필터링 UI |
| 부산 버스 실시간 API | Phase 9 | 실시간 버스 커버리지 확대 (부산 시내버스) |
| 인천 버스 실시간 API | Phase 9 연장 | 실시간 버스 커버리지 확대 (인천) |
| 대전 버스·지하철 실시간 API | Phase 9 연장 | 실시간 커버리지 확대 (대전) |
| 버스 노선도 뷰 | Phase 9 연장 | 버스 노선 실시간 현황 시각화 |
| 지하철 분기선 관리 | Phase 10 | 분기 노선 ETA 정확도 개선 |
| `useUrlState` | Phase 10 | URL 기반 UI 상태 관리 (딥링크 + 뒤로가기) |
| 여정 공유 (`is_public` + `/share/`) | Phase 10 | 비교근 여정 공유 기능 |

---

## Phase 9 연장 — 실시간 커버리지 확대 (2026-08-18 ~ 08-19)

### 버스 실시간 커버리지 확대
- **인천 버스 실시간 API 연동** (`IncheonBusService`)
  - 인천 광역시 버스 도착 조회 연동
  - `RealtimeTransitService.ts`에 인천 버스 통합
- **대전 버스·도시철도 실시간 API 도입**
  - 대전 시내버스 + 대전도시철도 실시간 도착 정보 통합
  - 지역 코드 분류 로직 확장 (`busRegionUtils.ts`)

### 버스 노선도 뷰 도입
- 버스 노선 실시간 현황을 시각화하는 노선도 시트/모달 구현
  - 지하철 `SubwayLineMapSheet`와 동일한 패턴으로 적용
- 버스·지하철 노선뷰 갱신 주기 및 UI 수정

### 지하철 실시간 개선
- 지하철 역간거리 정보 업데이트 및 파일 정리
- 실시간 도착 정보 시스템 리팩토링

---

## Phase 10 — 리팩토링 & 공유/URL 라우팅 (2026-08-26 ~ 08-28) — 최종

### 지하철 실시간 분기선 관리
- **지하철 분기선 분기 관리 방식 도입** (커밋 `334cdf1`)
  - 노선 분기 시 방향별 관리 로직 적용, ETA 정확도 개선
- **지하철 실시간 표기명 파싱 보완** (커밋 `f4b4e5e`)
  - SubwayMessageParser 보완 연장: 추가 엣지케이스 처리

### 프로젝트 리팩토링
- **대규모 리팩토링** (커밋 `01cd6d4`, `efca59e`)
  - 프로젝트 엔트리포인트 전반 코드 구조 정리
  - 컴포넌트 세부 분리, 불필요한 로그 제거, 타입 일관성 개선

### URL 라우팅 상태 관리 도입
- **`useUrlState` 훅 구현** (커밋 `eaa22cc`)
  - 브라우저 뒤로가기/앞으로가기 연동
  - 딥링크(Deep Link) 지원 — 특정 여정/장소/구간 URL 직접 접근
  - Next.js App Router와 Zustand 스토어 동기화

### 여정 공유 기능
- `is_public` 컬럼 + RLS 확장: 공개/비공개 전환
- `/share/[journeyId]` SSR 페이지: 비교근 사용자도 여정 확인 가능
- 공유 URL 복사 모달 구현

### 에이전트 명세 & 테스트 보강
- `AGENTS.md` 프로젝트 컨텍스트 최신화
- 테스트 파일 추가 (커밋 `c3270ec`)

---

*최종 업데이트: 2026-08-28 | **프로젝트 개발 종료 (Archived)** — 회고록: `docs/history/retrospective.md`*